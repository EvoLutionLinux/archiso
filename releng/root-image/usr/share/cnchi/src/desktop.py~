...
class DesktopAsk(Gtk.Box):
    ...
    def translate_ui(self, desktop):
        ...
        if desktop == 'nox':
            txt = _("This option will install Arch as command-line only system, " \
            "without any type of graphical interface. After the installation you can " \
            "customize Arch by installing packages with the command-line package manager.")
    ...
    def store_values(self):
        ...
        logging.info(_("Fu-rch will install Arch with the '%s' desktop") % self.desktop_choice)
        return True
