...
# leave at least 3.5GB for Lution when shrinking
_minimum_space_for_arch = 3500
...
class InstallationAlongside(Gtk.Box):
    ...
    def translate_ui(self):
        txt = _("Select the OS you would like Arch installed next to.")
        txt = '<span size="large">%s</span>' % txt
        self.label.set_markup(txt)

        txt = _("Arch Alongside Installation")
    ...
    def on_treeview_cursor_changed(self, widget):
        ...
        if self.min_size + _minimum_space_for_arch < self.max_size:
    ...
    def update_ask_shrink_size_labels(self, new_value):
        ...
        label_arch_size = self.ui.get_object("label_arch_size")
        label_arch_size.set_markup(str(int(self.max_size - new_value)) + " MB")
    ...
    def ask_shrink_size(self, other_os_name):
        ...
        # leave space for Lution
        self.available_slider_range = [ self.min_size, self.max_size - _minimum_space_for_arch ]
        ...
        label_arch = self.ui.get_object("label_arch")
        txt = "<span weight='bold' size='large'>Arch</span>"
        label_arch.set_markup(txt)
    ...
    def start_installation(self):
        ...
        if self.settings.get('install_bootloader'):
            ...
            logging.info(_("Fu-rch will install the bootloader of type %s in %s") % \
                (self.settings.get('bootloader_type'), self.settings.get('bootloader_device'))
        else:
            logging.warning("Lution will not install any boot loader")
