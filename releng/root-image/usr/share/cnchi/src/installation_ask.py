...
class InstallationAsk(Gtk.Box):
    ...
    def translate_ui(self):
        ...
        radio.set_label(_("Erase disk and install Arch"))
        ...
        if "windows" in self.otherOS.lower():
            radio = self.ui.get_object("alongside_radiobutton")
            label = self.ui.get_object("alongside_description")
            radio.set_label(_("Install Arch alongside %s") % self.otherOS)

            txt = _("Install Arch alongside %s") % self.otherOS
            txt = '<span weight="light" size="small">%s</span>' % txt
            label.set_markup(txt)
            label.set_line_wrap(True)
        ...
        radio.set_label(_("Choose exactly where Arch should be installed. (advanced)"))
    ...
    def store_values(self):
        ...
        if self.settings.get('use_luks'):
            logging.info(_("Arch installation will be encrypted using LUKS"))
            
        if self.settings.get('use_lvm'):
            logging.info(_("Arch will be installed using LVM volumes"))
